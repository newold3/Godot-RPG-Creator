...
const EQUIP_ITEM_BUTTON = preload("res://Scenes/EquipScene/equip_item_button.tscn")
...
func start() -> void:
	%EquipmentName.text = RPGSYSTEM.database.terms.search_message("Equip Equipment")
	started = false
	%Gear1.rotation = 0
	%Gear2.rotation = 0
	%EquipmentPanel.scale.y = 0.01
	%SmoothScrollContainer.scroll_vertical = 0
	var a = PI / 2.0
	GameManager.force_hide_cursor()
	var t = create_tween()
	t.set_parallel(true)
	t.tween_property(%EquipmentPanel, "scale:y", 1.0, 0.25).set_delay(0.2).set_trans(Tween.TRANS_SINE)
	t.tween_property(%Gear1, "rotation", a, 0.7).set_trans(Tween.TRANS_SINE).set_delay(0.15)
	t.tween_property(%Gear2, "rotation", -a, 0.7).set_trans(Tween.TRANS_SINE).set_delay(0.15)
	t.tween_callback(
		func():
			started = true
...
func _process(_delta: float) -> void:
	...
	if not current_button.has_focus() and manipulator != GameManager.MANIPULATOR_MODES.EQUIP_MENU_SUB_MENU:
...
func select_button(index: int) -> void:
	index = max(0, min(index, equip_button_container.get_child_count() - 1))
	equip_button_container.get_child(index).set_selected(true)

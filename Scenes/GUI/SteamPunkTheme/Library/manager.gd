...
func _process(_delta: float) -> void:
	...
	if not started or busy: return

	if not book_opened:
		var direction = ControllerManager.get_pressed_direction()
		if main_scene and direction:
			var current_book = main_scene.get_selected_book()
			var books: Array = main_scene.get_books()
			if not current_book: current_book = books[0]
			if current_book:
				books += [back_button]
				var next_control = ControllerManager.find_closest_control_in_list_by_direction(current_book, books, direction)
				if next_control:
		...
		elif ControllerManager.is_confirm_pressed(false, [], true, [KEY_ENTER]):
			if main_scene and _current_book:
				if _current_book == "main_book":

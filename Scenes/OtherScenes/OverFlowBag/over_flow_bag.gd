...
## Adds a new item record to the bag, stacking it with existing items if they share the same type, id, and level.
func add_item(type: int, id: int, amount: int, level: int = 1, auto_popup: bool = false, popup_prefix: String = "") -> void:
	for item in contents:
		if item.type == type and item.id == id and item.level == level:
			item.amount += amount
			return
...
## Registers the bag as the active interactive event when the player enters its area.
...
## Clears the bag from being the active interactive event when the player leaves its area.

...
func _on_event_list_item_selected(index: int) -> void:
	if index >= 0 and events.size() > index:
		var event: RPGEvent = events.get_event(index)
...
func select(id: int, emit_signal: bool = false, clear_filter: bool = false) -> void:
	...
	for i in events.get_events().size():
		current_event = events.get_event(i)
...
func select_object(obj: RPGEvent, emit_signal: bool = false, clear_filter: bool = false) -> void:
	select(obj.id, emit_signal, clear_filter)

...
## Initializes the base configuration for the Costumes command
func _ready() -> void:
	super()
	parameter_code = 127
...
## Renders the selected Costume resolving the UID properly
func _set_item_name() -> void:
	var uid = current_data.get("item_id", 1)
	
	if uid > 0 and uid < 1000000:
		uid = RPGSYSTEM.id_to_uid("costumes", uid)
		current_data.item_id = uid
		
	var item_data = RPGSYSTEM.get_data("costumes", uid) if uid > 0 else null
	
	if item_data:
		var classic_id = RPGSYSTEM.uid_to_id("costumes", uid)
		var size_pad = str(RPGSYSTEM.database.costumes.size()).length()
		var item_name = "%s:%s" % [
...
func _on_item_id_pressed() -> void:
	...
	var uid = current_data.get("item_id", 1)
	var classic_id = RPGSYSTEM.uid_to_id("costumes", uid) if uid > 0 else 1
	
	dialog.setup(RPGSYSTEM.database.costumes, classic_id, title, null)

...
func set_data() -> void:
	...
	current_data.skill_id = skill_id
	...
	if skill_id <= RPGSYSTEM.database.skills.size():
		%SkillSelected.text = "%s: %s" % [skill_id, RPGSYSTEM.database.skills[skill_id].name]
	else:
		%SkillSelected.text = "%s: " % skill_id
...
func _on_skill_selected_pressed() -> void:
	...
	dialog.setup(RPGSYSTEM.database.skills, current_data.skill_id, "Skills", null)

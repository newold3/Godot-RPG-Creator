...
## Initializes base dialog parameters and state
func _ready() -> void:
	super()
	parameter_code = 45
	fill_fixed_values()
...
func set_initial_values() -> void:
	...
	%SkillSelected.set_disabled(false)
...
func set_data() -> void:
	...
	var uid_skill = data.get("skill_id", 1)
	current_data.skill_id = uid_skill
	...
	var classic_skill_id = RPGSYSTEM.uid_to_id("skills", uid_skill) if uid_skill > 0 else 1
	if classic_skill_id > 0 and classic_skill_id < RPGSYSTEM.database.skills.size():
		%SkillSelected.text = "%s: %s" % [classic_skill_id, RPGSYSTEM.database.skills[classic_skill_id].name]
	else:
		%SkillSelected.text = "⚠ Invalid Skill"
...
#region Skill & Operation Logic
## Maps the internal state to Add mode
func _on_add_toggled(toggled_on: bool) -> void:
	if toggled_on: current_data.operand = 0



## Maps the internal state to Remove mode
func _on_remove_toggled(toggled_on: bool) -> void:
	if toggled_on: current_data.operand = 1



## Spawns the dialog to select a Skill resolving classic ID
func _on_skill_selected_pressed() -> void:
	var path = "res://addons/CustomControls/Dialogs/select_any_data_dialog.tscn"
	var dialog = RPGDialogFunctions.open_dialog(path, RPGDialogFunctions.OPEN_MODE.CENTERED_ON_MOUSE)
	dialog.database = RPGSYSTEM.database
	dialog.destroy_on_hide = true
	dialog.selected.connect(_on_skill_selected, CONNECT_ONE_SHOT)
	
	var uid = current_data.get("skill_id", 1)
	var classic_id = RPGSYSTEM.uid_to_id("skills", uid) if uid > 0 else 1
	
	dialog.setup(RPGSYSTEM.database.skills, classic_id, "Skills", null)



## Receives the classic ID from the dialog and stores it as a secure UID
func _on_skill_selected(skill_id: int, target) -> void:
	var uid = RPGSYSTEM.id_to_uid("skills", skill_id)
	current_data.skill_id = uid
	%SkillSelected.text = "%s: %s" % [skill_id, RPGSYSTEM.database.skills[skill_id].name]
#endregion

...
func _on_tree_exiting() -> void:
	if preset_need_saved:
		var f = FileAccess.open("res://addons/CustomControls/Resources/Other/script_presets.txt", FileAccess.WRITE)
...
func set_data() -> void:
	var script = parameters[0].parameters.get("script", "")
	%CodeEdit.set_text(script)


func build_command_list() -> Array[RPGEventCommand]:
	var commands = super()
	commands[-1].parameters.script = %CodeEdit.get_text()
	return commands

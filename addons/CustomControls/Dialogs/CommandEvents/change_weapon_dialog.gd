...
## Initializes the base configuration for the Weapons command
func _ready() -> void:
	super()
	parameter_code = 14
...
## Sets the operation to Increase and disables Include Equipment
...
## Sets the operation to Decrease and enables Include Equipment
...
## Submits the include equipment flag state
func _on_include_equipment_toggled(toggled_on: bool) -> void:
	current_data.include_equipment = toggled_on



## Modifies the selected level configuration based on item upgrades
...
## Renders the selected Weapon resolving the UID and handles level caps safely
func _set_item_name() -> void:
	var uid = current_data.get("item_id", 1)
	
	if uid > 0 and uid < 1000000:
		uid = RPGSYSTEM.id_to_uid("weapons", uid)
		current_data.item_id = uid
		
	var item_data = RPGSYSTEM.get_data("weapons", uid) if uid > 0 else null
	
	if item_data:
		var classic_id = RPGSYSTEM.uid_to_id("weapons", uid)
		var size_pad = str(RPGSYSTEM.database.weapons.size()).length()
		var item_name = "%s:%s" % [
			str(classic_id).pad_zeros(size_pad),
			item_data.name
		]
		%ItemID.text = item_name
		max_level = item_data.upgrades.max_levels
...
func _on_item_id_pressed() -> void:
	...
	var uid = current_data.get("item_id", 1)
	var classic_id = RPGSYSTEM.uid_to_id("weapons", uid) if uid > 0 else 1
	
	dialog.setup(RPGSYSTEM.database.weapons, classic_id, title, null)

...
## Checkbox to enable "Colorize" mode (Monochromatic tint based on luminosity).
@export var colorize_checkbox: CheckBox

## Checkbox to make the entire palette a single flat color (Uniform)
@export var uniform_checkbox: CheckBox

## Checkbox to apply changes to the 3 palettes (Primary, Secondary, Tertiary)
@export var apply_to_all_checkbox: CheckBox
...
## Logic for Uniform mode
var _uniform_enabled: bool = false

signal colors_changed(colors: Array[Color], apply_to_all_palettes: bool)
...
func _ready() -> void:
	...
	if uniform_checkbox:
		uniform_checkbox.toggled.connect(_on_uniform_toggled)
...
func _on_colorize_toggled(toggled: bool) -> void:
	...
	if toggled and _uniform_enabled and uniform_checkbox:
		uniform_checkbox.set_pressed_no_signal(false)
		_uniform_enabled = false
	
	_update_ui_state_for_modes()


func _on_uniform_toggled(toggled: bool) -> void:
	_uniform_enabled = toggled
	
	if toggled and _colorize_enabled and colorize_checkbox:
		colorize_checkbox.set_pressed_no_signal(false)
		_colorize_enabled = false
		
	_update_ui_state_for_modes()


func _update_ui_state_for_modes() -> void:
	var special_mode = _colorize_enabled or _uniform_enabled
	
	channel_option.disabled = special_mode
	
	_set_sliders_block_signals(true)
	
	if special_mode:
		# Set defaults for Special modes (Red, Saturation 50%, Normal/Mid Brightness)
...
func _on_parameters_changed(_value: float) -> void:
	...
	if not _colorize_enabled and not _uniform_enabled:
		_channel_adjustments[_current_channel_index].h = hue_slider.value
		_channel_adjustments[_current_channel_index].s = saturation_slider.value
		_channel_adjustments[_current_channel_index].b = brightness_slider.value
...
func _update_shader_visuals(colorize_h: float = 0.0, colorize_s: float = 0.0) -> void:
	...
	mat.set_shader_parameter("colorize_mode", _colorize_enabled or _uniform_enabled)

	if _colorize_enabled or _uniform_enabled:
...
func _update_colors() -> void:
	...
	if _uniform_enabled:
		...
		var target_v = (brightness_slider.value + 100.0) / 200.0
		
		_update_shader_visuals(target_h, target_s)
		
		var uniform_color = Color.from_hsv(target_h, target_s, target_v, 1.0)
		
		for i in range(_original_colors.size()):
			var final_col = uniform_color
			final_col.a = _original_colors[i].a 
			_display_colors.append(final_col)

	elif _colorize_enabled:
		# --- COLORIZE MODE LOGIC ---
		var target_h = (hue_slider.value + 180.0) / 360.0
		var target_s = (saturation_slider.value + 100.0) / 200.0
	...
	if apply_to_all_checkbox:
		apply_all = apply_to_all_checkbox.button_pressed
		
	colors_changed.emit(_display_colors, apply_all)
...
func _on_cancel_button_pressed() -> void:
	colors_changed.emit(_original_colors, false)
	queue_free()

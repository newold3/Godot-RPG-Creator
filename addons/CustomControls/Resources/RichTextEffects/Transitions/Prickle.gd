@tool
class_name RichTextEffectTransitionPrickle
...
func _process_custom_fx(char_fx: CharFXTransform) -> bool:
	...
	if visible_characters == -1:
		...
		_apply_prickle_effect(char_fx, char_time, effect_duration)
		return true
...
func _apply_prickle_effect(char_fx: CharFXTransform, char_time: float, duration: float):
